...
const createTextPost = (req, res, next) => {
  ...
  db.none(
    "INSERT INTO post(Post_Content, user_Id) VALUES(${Post_Content}, ${user_Id})",
    req.body
  )
    .then(() => {
      res.status(200).json({
...
const createPhotoPost = (req, res, next) => {
  req.body.user_Id = req.user.id;
  db.none(
    "INSERT INTO post(Post_Content, user_Id, img_url) VALUES(${Post_Content}, ${user_Id}, ${img_url})",
    req.body
  )
    .then(() => {
      res.status(200).json({
...
const getUserInfoForPost = (req, res, next) => {
  db.any(
    "SELECT * FROM users JOIN post ON post.user_id=users.id ORDER BY Time_Stamp desc"
  )
    .then(data => {
      res.status(200).json({
        data: data
...
const updatePost = (req, res, next) => {
  db.none("UPDATE post SET Post_Content=${Post_Content} WHERE id=${id}", {
...
const getUserPosts = (req, res, next) => {
  ...
  db.any(
    "SELECT * FROM users JOIN post ON post.user_id = users.id WHERE users.username=$1",
    user_username
  )
    .then(data => {
      res.status(200).json({
